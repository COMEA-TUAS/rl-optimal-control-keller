...
find $models_dir/run_models_dirs -name "log_*.csv" | while read f; do ll=$(tail -n 1 $f); echo $f","$ll; done | tr -d "\"" | sort -t "," -k 2 -g > $models_dir/all_run_models_figs_sorted_times.txt
